  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {initialEvents.filter(e => e.isActive).length > 0 ? initialEvents.filter(e => e.isActive).map((e) => (
        <div key={e._id} className="glass-card p-6 border-surface-border flex flex-col gap-4">
          ...
          <div className="pt-2">
            {isRegistered(e._id) ? (
              <div className="w-full py-2.5 rounded-xl bg-surface-border/10 border border-surface-border flex flex-col items-center">
                <span className="text-[7px] uppercase font-black text-text-muted/50 tracking-widest leading-none mb-1">Your Assigned Seat</span>
                <span className="text-lg font-black text-gold-dynamic tracking-tighter">{getSeat(e._id)}</span>
              </div>
            ) : (
              <button 
        ...
        <div className="col-span-full glass-card p-20 text-center border-dashed border-surface-border bg-transparent">
          ...
          <p className="text-text-muted uppercase font-black tracking-widest text-xs">No active events to join</p>
        </div>
      )}
    </div>
  );
