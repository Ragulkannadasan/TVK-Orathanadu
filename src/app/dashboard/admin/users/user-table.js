...
export default function UserTable({ initialUsers, currentUserEmail }) {
  ...
  const getRoleColors = (role) => {
    const colors = {
      ...
      Voter: "from-white/10 to-white/5 text-white/70 border-white/10",
    };
  ...
  return (
    <div className="w-full">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <div className="text-white font-semibold group-hover:text-[#FFD700] transition-colors">{user.name}</div>
                          {isSelf && (
                            <span className="text-[8px] bg-white/10 text-white/40 px-1.5 py-0.5 rounded-full uppercase font-bold tracking-tighter">You</span>
                          )}
                        </div>
                        <div className="text-[#FFD700]/70 text-[10px] font-bold tracking-tight">@{user.username || user.email.split('@')[0]}</div>
                        <div className="text-white/20 text-[9px]">{user.email}</div>
                      </div>
                    </div>
                  </td>
                <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-2">
                      ...
                      {isSelf && <span className="text-[8px] text-white/20 italic">You cannot demote yourself</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4 rounded-r-2xl text-right space-x-2">
                    <button
                      onClick={() => setSelectedUser(user)}
                      className="p-2 rounded-xl bg-white/5 text-white/70 hover:bg-white/10 transition-all"
                      title="View Profile"
                    >
                      <UserIcon size={18} />
                    </button>
            ...
            <div key={user._id} className="glass-card-no-hover p-4 animate-fade-in relative overflow-hidden group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div onClick={() => setSelectedUser(user)} className="cursor-pointer">
                      <div className="flex items-center gap-2">
                        <div className="text-white font-bold text-base truncate group-hover:text-[#FFD700] transition-colors">{user.name}</div>
                        {isSelf && (
                          <span className="text-[8px] bg-white/10 text-white/40 px-1.5 py-0.5 rounded-full uppercase font-bold tracking-tighter">You</span>
                        )}
                      </div>
                      <div className="text-[#FFD700]/70 text-xs font-bold tracking-tight">@{user.username || user.email.split('@')[0]}</div>
                    </div>
                  </div>
                  
                  <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-white/40 uppercase font-bold tracking-wider">
                    <div className="flex items-center gap-1"><MapPin size={10} className="text-[#FFD700]/40" /> {user.panchayat || "NO PANCHAYAT"}</div>
                    <div className="flex items-center gap-1"><Hash size={10} className="text-[#FFD700]/40" /> {user.boothNumber || "NO BOOTH"}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 pt-4 border-t border-white/5 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setSelectedUser(user)}
                    className="p-3 rounded-xl bg-white/5 text-white/70 hover:bg-white/10 active:scale-95 transition-all"
                  >
                    <UserIcon size={18} />
                  </button>
                  {!isSelf && (
                    <button
                      onClick={() => handleDelete(user._id)}
                      disabled={loading === user._id}
                      className="p-3 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white active:scale-95 transition-all disabled:opacity-50"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
                    </button>
                  )}
      ...
      {users.length === 0 && (
        <div className="text-center py-20 text-white/30">No members found.</div>
      )}

      {/* Profile Detail Modal (Instagram Style) */}
      {selectedUser && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="glass-card w-full max-w-lg overflow-hidden relative border-white/10 shadow-2xl">
            <button 
              onClick={() => setSelectedUser(null)}
              className="absolute top-4 right-4 p-2 rounded-full bg-white/5 text-white/50 hover:text-white hover:bg-white/10 transition-all z-10"
            >
              <X size={20} />
            </button>
                      ...
                      <div className="w-full h-full rounded-full bg-black p-1">
                        <div className="w-full h-full rounded-full bg-[#1a1a1a] flex items-center justify-center overflow-hidden border border-white/10">
                          {selectedUser.image ? (
                            ...
                            <span className="text-white font-bold text-3xl">{selectedUser.name?.[0]?.toUpperCase()}</span>
                          )}
                        </div>
                      </div>
                    </div>

                    <h2 className="text-2xl font-bold text-white display-font">{selectedUser.name}</h2>
                    <p className="text-white/40 text-sm mb-4">{selectedUser.email}</p>
                    {getRoleBadge(selectedUser.role, "lg")}
                  </div>

                  {/* Details Grid */}
                  <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                     <div className="glass-card-no-hover p-4 border-white/5 bg-white/2">
                        <div className="flex items-center gap-3 text-white/30 text-[10px] uppercase font-bold tracking-widest mb-1">
                           <Phone size={12} /> Mobile
                        </div>
                        <div className="text-white text-sm font-medium">{selectedUser.mobile || "Not Provided"}</div>
                     </div>
                     <div className="glass-card-no-hover p-4 border-white/5 bg-white/2">
                        <div className="flex items-center gap-3 text-white/30 text-[10px] uppercase font-bold tracking-widest mb-1">
                           <CreditCard size={12} /> Voter ID
                        </div>
                        <div className="text-white text-sm font-medium">{selectedUser.voterId || "Not Linked"}</div>
                     </div>
                     <div className="glass-card-no-hover p-4 border-white/5 bg-white/2">
                        <div className="flex items-center gap-3 text-white/30 text-[10px] uppercase font-bold tracking-widest mb-1">
                           <MapPin size={12} /> Panchayat
                        </div>
                        <div className="text-white text-sm font-medium">{selectedUser.panchayat || "Not Set"}</div>
                     </div>
                     <div className="glass-card-no-hover p-4 border-white/5 bg-white/2">
                        <div className="flex items-center gap-3 text-white/30 text-[10px] uppercase font-bold tracking-widest mb-1">
                           <Hash size={12} /> Booth
                        </div>
                        <div className="text-white text-sm font-medium">{selectedUser.boothNumber || "Not Assigned"}</div>
                     </div>
                  </div>

                  <div className="mt-6 flex items-center justify-between text-[10px] text-white/20 uppercase font-bold border-t border-white/5 pt-4">
      ...
      {roleUser && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-fade-in">
          <div className="glass-card w-full max-w-sm overflow-hidden relative border-white/10 shadow-2xl">
            <div className="p-8">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#FFD700] animate-pulse" />
                  <h3 className="text-white text-[10px] font-black uppercase tracking-[0.3em]">Change Designation</h3>
                </div>
                <button onClick={() => setRoleUser(null)} className="text-white/40 hover:text-white transition-colors p-2 -mr-2"><X size={20} /></button>
              </div>
              ...
              <div className="flex flex-col items-center mb-10">
                 <div className="w-20 h-20 rounded-full p-1 bg-gradient-to-tr from-[#FFD700] via-[#800000] to-[#FFD700] mb-4 shadow-2xl shadow-[#FFD700]/10">
                    <div className="w-full h-full rounded-full bg-black p-1">
                       <div className="w-full h-full rounded-full bg-[#1a1a1a] flex items-center justify-center overflow-hidden border border-white/5">
                          {roleUser.image ? (
                             <img src={roleUser.image} alt={roleUser.name} className="w-full h-full object-cover" />
                          ) : (
                             <span className="text-white font-bold text-2xl">{roleUser.name?.[0]?.toUpperCase()}</span>
                          )}
                       </div>
                    </div>
                 </div>
                 <div className="text-white font-black text-lg tracking-tight leading-none mb-1">{roleUser.name}</div>
                 <div className="text-[#FFD700] text-xs font-bold uppercase tracking-widest opacity-80">@{roleUser.username || roleUser.email.split('@')[0]}</div>
              </div>
                    ...
                    <button
                      ...
                      className={`w-full group relative overflow-hidden rounded-2xl transition-all duration-300 active:scale-[0.98] border
                        ${isSelected 
                          ...
                          : 'bg-white/5 text-white/40 hover:bg-white/10 hover:text-white border-white/5 hover:border-white/10'}
                      `}
                    >
              ...
              <button 
                ...
                className="mt-8 w-full py-2 text-[10px] font-black text-white/20 uppercase tracking-[0.2em] hover:text-[#FFD700] transition-all"
              >
                Close Window
              </button>
            </div>
