...
export default async function AdminGrievancesPage() {
  const session = await getSession(); // Deduplicate auth check
  await dbConnect();
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto pb-20">
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-maroon dark:text-gold text-xs font-black uppercase tracking-[0.3em]">Central Registry</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold text-foreground display-font mb-2">
