      ...
      {isNavigating && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-200">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#FFD700]">Loading...</span>
          </div>
        </div>
      )}

      {/* Header */}
      <header className="p-4 flex items-center justify-between bg-[#0a0a0a] border-b border-white/5 sticky top-0 z-10 backdrop-blur-md">
        <div className="flex items-center gap-4">
          <button 
            onClick={handleBack}
            className="p-2 rounded-full hover:bg-white/5 transition-all text-[#FFD700]"
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40 uppercase font-black tracking-widest">TVK Orathanadu</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className={`flex flex-col ${i % 2 === 0 ? "items-end" : "items-start"} animate-pulse`}>
                <div className="w-20 h-2 bg-white/5 rounded mb-2" />
                <div className={`h-10 ${i % 2 === 0 ? "w-48" : "w-64"} bg-white/5 rounded-2xl`} />
              </div>
            ))}
          </div>
        ) : messages.length === 0 ? (
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  {!isMe && (
                    <span className="text-[10px] font-bold text-[#FFD700] uppercase tracking-tighter">
                      {msg.senderName}
                    </span>
                  )}
                ...
                <div className={`px-4 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xl transition-all ${
                  isMe 
                    ? "bg-gradient-to-br from-[#900000] to-[#600000] text-white rounded-tr-none border border-white/10" 
      ...
      <div className="p-4 bg-[#0a0a0a] border-t border-white/5 relative">
        {sending && (
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-3 py-1 bg-[#800000] rounded-full flex items-center gap-2 shadow-lg animate-bounce">
            <Loader2 size={10} className="animate-spin text-[#FFD700]" />
            <span className="text-[8px] font-black uppercase tracking-widest text-[#FFD700]">Sending...</span>
          </div>
        )}
        <form onSubmit={handleSend} className="flex gap-2">
          <input
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-full px-4 py-2 text-sm focus:outline-none focus:border-[#FFD700]/30 transition-all placeholder:text-white/20 disabled:opacity-50"
            disabled={sending || isNavigating}
          />
          <button
            type="submit"
            disabled={!input.trim() || sending || isNavigating}
            className="w-10 h-10 rounded-full bg-[#800000] flex items-center justify-center text-[#FFD700] disabled:opacity-50 disabled:grayscale transition-all shadow-[0_0_15px_rgba(128,0,0,0.3)] active:scale-95"
          >
